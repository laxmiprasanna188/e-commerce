...
const Signup = () => {
    ...
    const submitHandler = async(e)=>{
        ...
        try {
            setLoading(true)
            const res = await axios.post(`http://localhost:5000/api/v1/user/register`, formData,{headers:{"Content-Type":"application/json"}})
            if(res.data.success){
                navigate('/verify')
        ...
        <div className="flex justify-center items-center min-h-screen bg-pink-100">
            <Card className="w-full max-w-sm">
                ...
                <CardFooter className="flex-col gap-2">
                    ...
                    <p className='text-gray-700 text-sm'>Already have account? <Link to={'/login'} className='hover:underline cursor-pointer text-pink-800'>Login</Link></p>
                </CardFooter>
            </Card>
